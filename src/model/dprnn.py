...
class Dual_RNN_model(nn.Module):
    ...
    def forward(self, mix_audio, **batch):
        ...
        audio = torch.stack([self.decoder(out[i]) for i in range(self.num_spks)], dim=1)

        mel_spec_after = self.get_melspec(audio)
        return {"separated_audios": audio, "fake_melspec": mel_spec_after}
