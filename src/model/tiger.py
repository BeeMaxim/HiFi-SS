...
class TIGER(nn.Module):
    def __init__(
        ...
        for i in range(self.nband):
            self.BN.append(nn.Sequential(nn.GroupNorm(1, self.band_width[i]*1, self.eps),
                                         nn.Conv1d(self.band_width[i]*1, self.feature_dim, 1)
                                        )
                          )
        ...
        for i in range(self.nband):
            self.mask.append(nn.Sequential(
                                           nn.PReLU(),
                                           nn.Conv1d(self.feature_dim, self.band_width[i]*1*num_sources, 1, groups=num_sources)
                                          )
                            )
    ...
    def forward(self, mix_audio, **batch):
        ...
        spec = torch.stft(input, n_fft=self.win, hop_length=self.stride, 
                          ...
                          return_complex=True)
        mel = self.get_melspec(input.unsqueeze(1))
        ...
        subband_feature = []
        for i in range(len(self.band_width)):
            subband_feature.append(self.BN[i](subband_spec_RI[i].view(batch_size*nch, self.band_width[i]*1, -1)))
        ...
        for i in range(self.nband):
            ...
            #this_mask_real = this_mask_real / this_mask_real_sum
            #print(this_mask_real[0, :, 0, :5])
