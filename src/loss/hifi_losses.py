...
def discriminator_loss(disc_real_outputs, disc_generated_outputs, mix_estimation):
    loss = 0
    r_losses = []
    g_losses = []
...
class DiscriminatorLoss(nn.Module):
    ...
    def forward(self, real_estimation, fake_estimation, mix_estimation, **batch):
        return {"discriminator_loss": discriminator_loss(real_estimation, fake_estimation, mix_estimation)[0]}
...
class BSSGeneratorLoss(nn.Module):
    ...
    def forward(self, real_melspec, fake_melspec, separated_audios, audios, ids, discriminator, **batch):
        ...
        for p in permutations:
            ...
            for b in range(B):
                sp = time.time()
                cur_loss[b] += -self.si_snr_loss(separated_audios[b, p, :], audios[b]) * 4
                # print(p)
        ...
        losses["generator_loss"] = losses["snr_loss"] * 4
