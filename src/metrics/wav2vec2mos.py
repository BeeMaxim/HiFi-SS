...
class Wav2Vec2MOS(nn.Module):
    ...
    def __init__(self, path, device='cpu', freeze=True):
        super().__init__()
        ...
        self.load_state_dict(extract_prefix("model.", torch.load(path, map_location=torch.device(device))["state_dict"]))
        self.eval()
        self.to(device)
    ...
    def calculate(self, samples):
        ...
        for s in samples:
            ...
            with torch.no_grad():
                res = self.forward(x.to(self.device)).mean()
